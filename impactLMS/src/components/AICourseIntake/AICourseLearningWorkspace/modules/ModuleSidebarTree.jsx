...
export default function ModuleSidebarTree({ modules, activeModuleId, activeTopicIndex, completedTracks, onSelectTopic }) {
  ...
  const verifyLockStatus = (currentModId, currentTopicIdx) => {
    ...
    if (targetPreviousTopicIdx < 0) {
      ...
      const prevModObj = modules.find(m => m.moduleId === targetPreviousModId);
    ...
    <div style={{ width: '320px', background: '#070a12', borderRight: '1px solid rgba(255,255,255,0.03)', display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '1rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {modules.map((mod, index) => {
          const isModuleExpanded = expandedModules[mod.moduleId];
          ...
          return (
            <div key={mod._id || index} style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.01)', borderRadius: '0.5rem', overflow: 'hidden' }}>
              ...
              <div 
                ...
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.85rem 1rem', cursor: 'pointer', background: activeModuleId === mod.moduleId ? 'rgba(6,182,212,0.02)' : 'transparent' }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <BookOpen size={14} style={{ color: activeModuleId === mod.moduleId ? '#06B6D4' : '#64748b' }} />
                  <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#fff' }}>Module {index + 1}: {mod.moduleName}</span>
                </div>
              ...
              {isModuleExpanded && (
                <div style={{ padding: '0.2rem 0.5rem 0.5rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', borderLeft: '1px dashed #1e293b', marginLeft: '1.5rem' }}>
                  {(mod.topics || []).map((topic, idx) => {
                    const isLocked = verifyLockStatus(mod.moduleId, idx);
                    const isActive = activeModuleId === mod.moduleId && activeTopicIndex === idx;
                    const isFinished = completedTracks[`mod-${mod.moduleId}-topic-${idx}`];

                    return (
                      <button
                        key={idx}
                        disabled={isLocked}
                        onClick={() => onSelectTopic(mod.moduleId, idx)}
                        style={{
