...
export default function ModuleSidebarTree({ modules, activeModuleId, activeTopicIndex, completedTracks, onSelectTopic }) {
  ...
  const toggleModuleAccordion = (dayId) => {
    setExpandedModules(prev => ({ ...prev, [dayId]: !prev[dayId] }));
  };
  ...
  return (
    <div style={{ width: '320px', background: '#070a12', borderRight: '1px solid rgba(255,255,255,0.03)', display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '1rem' }}>
      <h3 style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '1rem', paddingLeft: '0.5rem' }}>Course Syllabus Directory</h3>
      
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {modules.map((mod, index) => {
          const isModuleExpanded = expandedModules[mod.dayId];
          return (
            <div key={mod.dayId} style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.01)', borderRadius: '0.5rem', overflow: 'hidden' }}>
              
              {/* 🚀 FIXED: CHANGED "DAY" TO "MODULE" */}
              <div 
                onClick={() => toggleModuleAccordion(mod.dayId)}
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.85rem 1rem', cursor: 'pointer', background: activeModuleId === mod.dayId ? 'rgba(6,182,212,0.02)' : 'transparent', transition: 'all 0.2s' }}
              >
              ...
              {/* Child Inner Topics List Frame Layout */}
              {isModuleExpanded && (
                <div style={{ padding: '0.2rem 0.5rem 0.5rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', borderLeft: '1px dashed #1e293b', marginLeft: '1.5rem', marginBottom: '0.5rem' }}>
